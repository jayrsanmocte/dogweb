...
<>
    ...
    <div className='grid col-12 container' >
      <div className="row">
      { 
      dogbreeds.map((dogbreed)=>(
        <div key={dogbreed[0]}   className= {`grid-item btn-custom ${dogbreed[0]}`} onClick={() => handleButtonClick(dogbreed[0], dogbreed[1])}>
              <div className="paws1 fas fa-paw me-3 hvr-icon-buzz-out" />
              <span className="button-text">{dogbreed[0]}</span>
        </div>  
      ))
      }
      </div>
